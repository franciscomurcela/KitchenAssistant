...
var MMIClient = (function () {
    ...
    // Função para enviar logs ao servidor Flask
    async function sendLogToProxyLogger(logData) {
        try {
            ...
            if (!response.ok) {
                console.error("Failed to send log to proxy logger:", response.statusText);
            }
        } catch (error) {
            console.error("Failed to send log to proxy logger:", error);
        }
    }
    ...
    MMIClient.prototype.sendToIM = function (lce) {
        ...
        sendLogToProxyLogger({ log: lce.toString() }); // Enviar log para o proxy logger
...
var MMIClientSocket = (function () {
    function MMIClientSocket(address) {
    ...
    Object.defineProperty(MMIClientSocket.prototype, "OnOpen", {
        get: function () { return this.onOpen.expose(); },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(MMIClientSocket.prototype, "OnMessage", {
        get: function () { return this.onMessage.expose(); },
        enumerable: true,
        configurable: true
    });
